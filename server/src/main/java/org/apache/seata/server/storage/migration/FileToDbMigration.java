...
/**
 * Migration from File storage mode to DB storage mode.
 *
 * @author xingfudeshi@gmail.com
 */
public class FileToDbMigration extends AbstractStorageMigration {

    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(FileToDbMigration.class);

    private static final int DEFAULT_READ_SIZE = 100;

    public FileToDbMigration() {
    ...
    public boolean validate() {
        try {
            ...
            LOGGER.info("File to DB migration validation passed");
            return true;
        } catch (Exception e) {
            LOGGER.error("Failed to validate File to DB migration", e);
            return false;
        }
    }
    ...
    protected boolean createBackup() {
        LOGGER.info("File to DB migration: Creating backup of source file data is recommended");
        return true;
    }

    @Override
    protected void removeBackup() {
        LOGGER.info("File to DB migration cleanup completed");
    }
}
